<script lang="ts">
	import { BOOK_LIST, type GameData } from '$lib';
	...
	const computeTableData = (
			...
			for (const [book, odds] of Object.entries(latestHistory.odds)) {
				...
				if (modifyMGM && book === 'betmgm' && adjustedOdds) {
					const newImpliedProbability =
						1 + adjustedOdds / 200 - Math.sqrt(adjustedOdds ** 2 + 400 * adjustedOdds) / 200;
		...
		filteredOdds = latestOdds.filter((player) => player.odds[fairBook] || player.odds[targetBook]);
	...
	const getBookEdges = () => {
		...
		for (const book of BOOK_LIST) {
			bookEdges[book] = latestOdds.reduce(
				(acc, player) => acc + (player.odds[book] ? impliedProb(player.odds[book]) : 0),
				0
			);
		}
		return bookEdges;
	...
	const getImpliedProbs = () => {
		...
		for (const book of BOOK_LIST) {
			...
			for (const player of latestOdds) {
				impliedProbs[book][player.player] = player.odds[book]
					? impliedProb(player.odds[book]) / bookEdges[book]
					: 0;
			}
		}
		return impliedProbs;
	...
	<tbody>
		{#each filteredOdds as { player, odds }, i}
			...
			<tr class={`${scaledBets[player] > 0 ? 'bg-blue-50' : ''}`}>
				...
				<td
					>{targetOdds > 0
						? Math.round(impliedProbs[targetBook][player] * 1000) / 10 + '%'
						: '-'}</td
				>
