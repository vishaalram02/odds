...
<div class="container mx-auto p-4">
	<div class="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3">
		<div>
			...
			<label class="mb-2 block text-sm font-medium">Select Date:</label>
			<select class="w-full rounded border p-2" bind:value={date}>
		...
		{#if gameData.length > 0}
			<div>
				<!-- svelte-ignore a11y_label_has_associated_control -->
				<label class="mb-2 block text-sm font-medium">Select Game:</label>
				<select class="w-full rounded border p-2" bind:value={selectedGameId}>
			...
			{#if currentGame}
				<div>
					<!-- svelte-ignore a11y_label_has_associated_control -->
					<label class="mb-2 block text-sm font-medium">Select Player:</label>
					<select class="w-full rounded border p-2" bind:value={selectedPlayer}>
	...
	{#if selectedPlayer && chartData.length > 0}
		<div class="mb-6">
			<h2 class="mb-4 text-xl font-bold">Odds History for {selectedPlayer}</h2>
			<div class="h-[400px]">
				{#key chartData}
					<PlayerChart {chartData} bind:modifyMGM />
				{/key}
			</div>
		</div>
	{/if}
